...
namespace Checador
{
    public partial class crmVersion : Form
    {
...
//--------------------------------------------------------------- > crear y dar formato de pdf
        ...
        private void dtg_vista_CellMouseDoubleClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            try
            {
                SqlCeCommand cm = new SqlCeCommand("SELECT enlace FROM pagina WHERE Id_pag = @iden", datos.cn);
                cm.Parameters.AddWithValue("@iden", dtg_vista.Rows[e.RowIndex].Cells[0].Value.ToString());
              ...
              //  MessageBox.Show(dt.Rows[0][0].ToString().Trim());
               ...
               System.Diagnostics.Process.Start( dt.Rows[0][0].ToString().Trim());

            }
            catch (Exception exx)
            {
                MessageBox.Show("A ocurrido un error al abrir el navegador, " + exx.Message);
            }
        }
        ...
        private void btn_crm_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                foreach (DataGridViewRow row in dtg_vista.SelectedRows)
                {
                    ...
                    value10 = row.Cells[3].Value.ToString();                    // Hora de revision
                    ...
                    if (value3 == "tercero") 
                    {
                         Asunto = "Actualizar pagina B: " + value1 + " - " + value2;
                    }
                    else
                    {
                         Asunto = "Actualizar pagina A: " + value1 + " - " + value2;
                    }
        ...
        public void general()
        {
            try
            {
                //MessageBox.Show(cb_combo.SelectedValue.ToString());
                SqlCeCommand cm = new SqlCeCommand("SELECT a.Id_pag AS ID, b.gasolinera AS GRUPO, a.nombre AS NOMBRE, a.hora AS HORA, b.servicio AS TIPO FROM chec a INNER JOIN pagina b ON a.id_pag = b.Id_pag WHERE fecha = '" + DateTime.Now.ToString("dd/MM/yy") + "' and a.version = '" + @cb_combo.SelectedValue.ToString()+"' ORDER BY b.gasolinera", datos.cn);
        ...
        public void clientes()
        {
            try
            {
                SqlCeCommand cm = new SqlCeCommand("SELECT a.Id_pag AS ID, b.gasolinera AS GRUPO, a.nombre AS NOMBRE, a.hora AS HORA, b.servicio AS TIPO FROM chec a INNER JOIN pagina b ON a.id_pag = b.Id_pag WHERE a.fecha = '" + DateTime.Now.ToString("dd/MM/yy") + "'and a.servicio = 'cliente' and a.version = '" + @cb_combo.SelectedValue.ToString() + "'", datos.cn);
                //  cm.Parameters.AddWithValue("@id", txt_idbusc.Text);
                //  cm.Parameters.AddWithValue("@nombre", txt_idbusc.Text);
                SqlCeDataAdapter da = new SqlCeDataAdapter(cm);
                DataTable dt = new DataTable();
        ...
        public void terceros()
        {
            try
            {
                SqlCeCommand cm = new SqlCeCommand("SELECT a.Id_pag AS ID, b.gasolinera AS GRUPO, a.nombre AS NOMBRE, a.hora AS HORA, b.servicio AS TIPO FROM chec a INNER JOIN pagina b ON a.id_pag = b.Id_pag WHERE a.fecha = '" + DateTime.Now.ToString("dd/MM/yy") + "' and a.servicio = 'tercero' and a.version = '" + @cb_combo.SelectedValue.ToString() + "'", datos.cn);
                //  cm.Parameters.AddWithValue("@id", txt_idbusc.Text);
                //  cm.Parameters.AddWithValue("@nombre", txt_idbusc.Text);
                SqlCeDataAdapter da = new SqlCeDataAdapter(cm);
                DataTable dt = new DataTable();
        ...
        private void cargarCombo()
        {
            try
            {
                ...
                SqlCeCommand cm = new SqlCeCommand("SELECT version FROM chec WHERE fecha = '"+ DateTime.Now.ToString("dd/MM/yy") + "' GROUP BY version;", datos.cn);
                SqlCeDataAdapter da = new SqlCeDataAdapter(cm);
                DataTable dt = new DataTable();
                da.Fill(dt);
                ...
                if (dt.Rows.Count > 0)
                {
                    cb_combo.DisplayMember = "version";

                    cb_combo.DataSource = dt;
                    general(); //------------------------------------------- llama a la funcion para cargar el datagrid
                }
                else
                {
                }
        ...
        private void cb_combo_SelectedIndexChanged(object sender, EventArgs e)
        {
            cambios();
        }
    }
