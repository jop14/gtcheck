...
namespace Checador
{
    public partial class repConDia : Form
    {
        ...
        private void crear_pdf()
        {
            ...
            if (filename.Trim() != "")
            {
               ...
               // string remito = "Autorizo: Roberto espinoza";
                ...
                doc.Add(imagen);
                Chunk chunk = new Chunk("Reporte del dia "+ dtp_ini.Text, FontFactory.GetFont("ARIAL", 20, iTextSharp.text.Font.BOLD));
                doc.Add(new Paragraph(chunk));
                doc.Add(new Paragraph("                       "));
               // doc.Add(new Paragraph("                       "));
                doc.Add(new Paragraph("------------------------------------------------------------------------------------------"));
                doc.Add(new Paragraph("Reporte con los datos generados de un dia en especifico para conocer el estado de las paginas para clientes y terceros."));
                doc.Add(new Paragraph("Consulta de la fecha "+ dtp_ini.Text ));
        ...
        private void button1_Click(object sender, EventArgs e)
        {
          //  MessageBox.Show("Today is " + fecha1.ToString("dd/MM/yy") + "."); mostrar la cadena de la fecha
            try
            {
                ...
                SqlCeCommand cm = new SqlCeCommand("SELECT id_pag AS IDENTIFICADOR, nombre AS NOMBRE, estado AS ESTADO, version AS VERSION,fecha AS FECHA, dia AS DIA, hora AS HORA FROM chec where fecha = @fecha1", datos.cn);
