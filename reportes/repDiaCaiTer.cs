...
namespace Checador
{
    public partial class repDiaCaiTer : Form
    {
        ...
        private void pag_caidasdia_Load(object sender, EventArgs e)
        {
            try
            {
                SqlCeCommand cm = new SqlCeCommand("SELECT a.Id_pag AS ID,b.gasolinerA AS GRUPO, a.nombre AS NOMBRE, b.servicio AS TIPO, a.hora AS HORA, b.enlace AS ENLACE, b.linkGas AS [INDEX] FROM chec a INNER JOIN pagina b ON a.id_pag = b.Id_pag WHERE fecha = '" + DateTime.Now.ToString("dd/MM/yy") + "' and estado = 'true' and b.tipo = 'tercero' ORDER BY b.gasolinera", datos.cn);
        ...
        private void crear_pdf()
        {
            ...
            if (filename.Trim() != "")
            {
               ...
               // string remito = "Autorizo: Roberto espinoza";
                ...
                Chunk chunk = new Chunk("Reporte diario de paginas caidas de usuarios tipo Terceros", FontFactory.GetFont("ARIAL", 20, iTextSharp.text.Font.BOLD));
                doc.Add(new Paragraph(chunk));
                doc.Add(new Paragraph("                       "));
                // doc.Add(new Paragraph("                       "));
                doc.Add(new Paragraph("--------------------------------------------------------------------------------------------------------------------------------------------"));
                doc.Add(new Paragraph("Reporte con los datos generados de paginas caidas de Terceros del dia."));
            ...
            try
            {
                ...
                if (filename.Trim() != "")
                {
                      ...
                      FileShare.ReadWrite))
                    {
                        ...
                        Chunk chunk = new Chunk("Reporte diario de paginas caidas de usuarios de tipo Terceros", FontFactory.GetFont("ARIAL", 20, iTextSharp.text.Font.BOLD));
                        doc.Add(new Paragraph(chunk));
                        doc.Add(new Paragraph("                       "));
                        // doc.Add(new Paragraph("                       "));
                        doc.Add(new Paragraph("--------------------------------------------------------------------------------------------------------------------------------------------"));
