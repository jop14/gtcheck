...
namespace Checador
{
    public partial class repDiario : Form
    {
        ...
        private void pag_caidasdia_Load(object sender, EventArgs e)
        {
            try
            {
                SqlCeCommand cm = new SqlCeCommand("SELECT id_pag AS IDENTIFICADOR, nombre AS NOMBRE, estado AS ESTADO, version AS VERSION,fecha AS FECHA, hora AS HORA FROM chec WHERE fecha = '"+ DateTime.Now.ToString("dd/MM/yy") + "'", datos.cn);
        ...
        private void crear_pdf()
        {
            ...
            if (filename.Trim() != "")
            {
               ...
               // string remito = "Autorizo: Roberto espinoza";
                ...
                Chunk chunk = new Chunk("Reporte de paginas del dia de hoy: "+ DateTime.Now.ToString("dd/MM/yy"), FontFactory.GetFont("ARIAL", 20, iTextSharp.text.Font.BOLD));
                doc.Add(new Paragraph(chunk));
                doc.Add(new Paragraph("                       "));
               // doc.Add(new Paragraph("                       "));
                doc.Add(new Paragraph("------------------------------------------------------------------------------------------"));
                doc.Add(new Paragraph("Reporte con los datos generados del dia de hoy para conocer el estado de las paginas para clientes y terceros."));
