...
namespace Checador
{
    public partial class repDiaActCli : Form
    {
        ...
        private void pag_caidasdia_Load(object sender, EventArgs e)
        {
            try
            {
                SqlCeCommand cm = new SqlCeCommand("SELECT id_pag AS IDENTIFICADOR, nombre AS NOMBRE, estado AS ESTADO, version AS VERSION,fecha AS FECHA, hora AS HORA FROM chec WHERE fecha = '" + DateTime.Now.ToString("dd/MM/yy") + "' and estado = 'false' and servicio = 'cliente'", datos.cn);
        ...
        private void crear_pdf()
        {
            ...
            if (filename.Trim() != "")
            {
               ...
               // string remito = "Autorizo: Roberto espinoza";
                ...
                Chunk chunk = new Chunk("Reporte diario de paginas activas para Clientes del dia ", FontFactory.GetFont("ARIAL", 20, iTextSharp.text.Font.BOLD));
                doc.Add(new Paragraph(chunk));
                doc.Add(new Paragraph("                       "));
               // doc.Add(new Paragraph("                       "));
                doc.Add(new Paragraph("------------------------------------------------------------------------------------------"));
                doc.Add(new Paragraph("Reporte que se genera con los datos de paginas activas del dia de hoy con todos los usuarios de tipo Cliente."));
